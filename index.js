...
WebpackAssetsManifest.prototype.toString = function(){
    ...
    let extractJsCss = this.extractJsCss,
        moduleAssets = this.moduleAssets;
    let _manifests = Object.assign({},moduleAssets);
    /*是否抽取css和JS*/
    if(extractJsCss){
      for(let key in _manifests){
        let filename= _manifests[key];
        if(path.extname(filename)=='.js' || path.extname(filename)=='.css'){
            this.versionFiles.push(filename);
        }
      }
    }
